...
# Get the user's favorite team from the list of Premier League teams
favorite_team = input("Enter your favorite Premier League team: ")

# Check if the user's favorite team is in the Premier League
if favorite_team not in teams:
    print("Sorry, your team is not in the Premier League")
    exit()
...
for i in range(num_matches):
    ...
    for j in range(0, len(teams), 2):
        ...
        print(f"{home_team} {home_score} - {away_score} {away_team}: {top_scorer[0]} is the top scorer with"
              f" {top_scorer[1]} goals")
